...
public final class ZstdBundleSelfTest {
    ...
    private static Properties manifest(byte[] data,byte[] xz) {
        ...
        p.setProperty("file.0.compression","zstd");p.setProperty("file.0.storedSize",""+xz.length);p.setProperty("file.0.storedSha256",RuntimeBundle.sha256(xz));p.setProperty("config.disk","images/system.qcow2");return p;
    }
    ...
    public static void main(String[] args)throws Exception {
        ...
        try {
            ...
            var compressed=new ByteArrayOutputStream();try(var out=ZstdCompression.encode(compressed)){out.write(data);}byte[] xz=compressed.toByteArray();
            ...
            byte[] huge=HexFormat.of().parseHex("28b52ffd0090010000"); // empty frame requests a 256 MiB window
            rejected(()->{try(var input=ZstdCompression.decode(new ByteArrayInputStream(huge))){input.readAllBytes();}});
            var old=manifest(data,xz);old.setProperty("schema","1");rejected(()->install(root.resolve("old"),old,xz,()->{}));
            System.out.println("ZSTD_BUNDLE_OK exact bytes, cached integrity, truncation/corruption, size limits, decoder memory cap, cancellation cleanup, schema compatibility");
