...
class PackageAndroidImages {
    ...
    public static void main(String[] args)throws Exception {
        if(args.length!=7)throw new IllegalArgumentException("IMAGES ARCH SYSTEM.qcow2.xz|SYSTEM.qcow2.zst DOWNLOAD_HTTPS_URL NEW.zip NEW.properties BASE_MOD.jar");
        ...
        String compression=xz.toString().endsWith(".zst")?"zstd":"xz",suffix=compression.equals("zstd")?".zst":".xz";
        ...
        try {
            try(var zip=new ZipOutputStream(Files.newOutputStream(tmp))) {
                zip.setLevel(9);
                for(int i=0;i<3;i++) {
                    Path raw=images.resolve(names[i]),stored=i==0?xz:raw;String key="file."+i+".",entry=i==0?names[i]+suffix:names[i];
                    ...
                    m.setProperty(key+"compression",i==0?compression:"none");m.setProperty(key+"storedSize",""+Files.size(stored));m.setProperty(key+"storedSha256",storedHash);
                ...
                zip.putNextEntry(BundleMetadata.entry("manifest.properties"));BundleMetadata.writeProperties(m,zip);zip.closeEntry();
