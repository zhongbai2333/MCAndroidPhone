...
本次 Java 迁移尚未发布；本地 `sh gradlew build` 的 JAR 可放入已安装的 Minecraft 26.1.2 / NeoForge 26.1.2.76。它复用游戏的 Java 25；测试图只需 `backend=pattern`。此前 GitHub 预览版仍是旧 Python 实现，不能用来验收移除 Python 依赖。

此前 CI 干净环境受 NeoForge 依赖元数据端点 HTTP 502 阻断。本次 M4 本机已完成 Gradle 构建，使用了已有的部分依赖缓存；这个结果不代表远端 CI 或完全无缓存的构建已恢复。`quick` 和安装好的游戏使用预览版 JAR 不依赖这个构建端点。[详细验证记录](portable-validation.md)
